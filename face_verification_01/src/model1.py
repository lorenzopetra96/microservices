...
dataframe_src = root_dir + '/df/ArcFace_v3.csv'
...
def compute_distance(img1, img2, detector='skip', metric='cosine'):
    """
    Compute distance between two images by using ArcFace model
...
def face_verification(json):
    ...
    try:
        doc_pic = conversion_utils.b64_to_numpy(doc_pic)  # , "model1_img1.jpg")
        usr_pic = conversion_utils.b64_to_numpy(usr_pic)  # , "model1_img2.jpg")
    except:
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        # app.run(host='127.0.0.1', port=5001) # development
        serve(app, host="0.0.0.0", port=5001)  # production
    else:
        print('Running...')
